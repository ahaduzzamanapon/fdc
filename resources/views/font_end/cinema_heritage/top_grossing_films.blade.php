...
@section('body')
    ...
    @php
        $heroRightImg = (!empty($page->banner_image) && !\Illuminate\Support\Str::contains($page->banner_image, 'unsplash.com'))
            ...
            : asset('portal/image/hero_top_grossing.svg');
    @endphp
            ...
            <div class="col-md-12">
                <div class="row align-items-center">
                    <div class="col-md-7 heroLeft">
                        <span class="heroTitle text-white">{{ $page->title ?? 'বছরভিত্তিক সর্বোচ্চ ব্যবসাসফল সিনেমা' }}</span>
                    ...
                    <div class="col-md-5 text-center my-3 my-md-0">
                        <img class="heroImg img-fluid" src="{{ $heroRightImg }}" alt="hero_top_grossing.svg">
                    </div>
                </div>
            </div>
        </div>
            ...
            @if(isset($page->items) && count($page->items) > 0)
                <div class="row g-4" id="heritage-grid">
                    @foreach($page->items as $index => $item)
                        @php
                            $defaultImg = 'https://images.unsplash.com/photo-1517604931442-7e0c8ed2963c?w=600&auto=format&fit=crop&q=80';
                ...
                @if(count($page->items) > 9)
                    ...
                    <div id="infinite-scroll-sentinel" class="text-center py-4 mt-4">
                        <div id="auto-load-spinner" class="spinner-border text-success d-none" role="status">
